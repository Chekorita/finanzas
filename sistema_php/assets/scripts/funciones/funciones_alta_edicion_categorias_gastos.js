...
document.addEventListener("focusout", function(e){
    const evt_nombre = e.target.closest("#nombre_categoria_gasto");
    if(evt_nombre){
...
function validar_actualizacion(id){
    if(valida_datos_categoria()){
        Swal.fire({
            title: '¿Estás seguro?',
            text: "Se actualizarán los datos de la categoría de gastos",
            icon: 'warning',
...
function validar_insercion(){
    if(valida_datos_categoria()){
        Swal.fire({
            title: '¿Estás seguro?',
            text: "Se insertará una nueva categoría de gastos",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonText: 'Aceptar',
            cancelButtonText: 'Cancelar',
...
function valida_datos_categoria(){
    let campos = [];
    let input_nombre = document.querySelector("#nombre_categoria_gasto");
    let valor_nombre = input_nombre.value ? limpiar_cadena_parcial(input_nombre.value) : "";
    input_nombre.value = valor_nombre;
    var campo = {
        valor: valor_nombre,
        contenedor: "#nombre_categoria_gasto",
        ayuda: "#ayuda_nombre_gasto",
        valido: (valor_nombre.length > 0 && valor_nombre !== "") ? true : false,
        mensaje: "Debe ingresar el nombre del gasto"
    };
    campos.push(campo);
    let validos = true;
...
async function insertar_categoria(){
    ...
    var contenedor_bloqueo = document.getElementById("contenedor-bloquea");
    let valor_nombre = document.getElementById("nombre_categoria_gasto").value;
    let valor_tipo = document.getElementById("sel_tipo_gasto").value;
    ...
    datos.append("funcion", "guardar_categoria_gasto");
    datos.append("parametros", JSON.stringify(parametros));
    contenedor_status.innerHTML = getAnimacionCarga("Insertando categoria de gasto...","secondary");
...
async function actualizar_categoria(id){
    ...
    datos.append("funcion", "actualizar_categoria_gasto");
    datos.append("parametros", JSON.stringify(parametros));
    contenedor_status.innerHTML = getAnimacionCarga("Actualizando categoria de gasto...","secondary");
    contenedor_bloqueo.classList.remove("cargando_oculto");
    contenedor_bloqueo.classList.add("cargando");
    fetch(url, {
        method: "POST",
